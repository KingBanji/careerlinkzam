  ...
  return (
    <div className="space-y-6" id="job-list-tab">
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (window.innerWidth < 1024) {
                        setTimeout(() => {
                          const el = document.getElementById("job-details-pane");
                          if (el) {
      ...
      {showApplyModal && selectedJob && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs font-sans" id="apply-modal">
          <div className="bg-white rounded-3xl max-w-lg w-full flex flex-col max-h-[90vh] shadow-2xl overflow-hidden relative border border-brand-border">
            {/* Close Button top-right */}
            <button
              type="button"
              onClick={() => setShowApplyModal(false)}
              className="absolute top-4 right-4 z-10 p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-all active:scale-95 cursor-pointer"
            >
              <X className="h-5 w-5" />
            </button>
